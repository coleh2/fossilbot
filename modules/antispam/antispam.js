...
function updateRecords(evt, server_config) {
    ...
    for(let i = 0, x; i < listOfSpecificEntryKeys.length; i++) {
        x = listOfSpecificEntryKeys[i];
        for(let iz = 0, y; iz < cooldowns.specific[evt.d.guild_id][evt.d.author.id][x].length; iz++) {
    ...
    //...for mass
    for(let i = 0, x; i < cooldowns.mass[evt.d.guild_id][evt.d.author.id].length; i++) {
    ...
    if(cooldowns.last_everyone_reset_time[evt.d.guild_id] < currentTime - (server_config.cooldown_e_t||3600000)) {
        ...
        for (let i = 0; i <= keysOfEveryoneCooldown.length; i++){
    ...
    if(evt.d.mentions.length > 0) {
        //for specific people:
        for(let i = 0; i < evt.d.mentions.length; i++) {
