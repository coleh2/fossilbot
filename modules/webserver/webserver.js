...
app.post("/gutekanstTweet", function(req,resp) {
    ...
    for(let i = 0; i < arrayOfPhrasesWhichIndicateASnowDay.length; i++) {
...
app.get("/data", function (req, resp) {
    ...
    if (q.src == "steam") {
        if (q.type == "lb") {
            try {
                uS = c.filter(x => { return x[q.src].data[q.mode]; });
        ...
        if (q.type == "lb") {
            try {
                uS = c.filter(x => { try { return x[q.src].data[q.mode]; } catch (e) { console.error(e); } });
